window.onload = function() {
  ...
  // Create instance of a database (db) object for us to store our database in
  let db;
  ...
  function init() {
    ...
    for (let i = 0; i < videos.length; i++) {
      ...
      request.onsuccess = function() {
        ...
        if (request.result) {
          // Grab the videos from the IDB and display them using displayVideo()
          console.log('taking videos from IDB');
          displayVideo(request.result.mp4, request.result.webm, request.result.name);
        } else {
          // Fetch the videos form the network
  ...
  function fetchVideoFromNetwork(video) {
    ...
    // Only run when both promises fulfilled
  ...
  function storeVideo(mp4Blob, webmBlob, name) {
    // Open transaction, get object store
    // Make it 'readwrite' so we can write to the IDB
  ...
  request.onsuccess = function() {
    ...
    // Store the opened database object in the db variiable.
  ...
  request.onupgradeneeded = function(e) {
    // Grab an reference to the opend database
  ...
  // Repo should be folders deep:
  // e.g. javascript/apis/client-side-storage/cache-sw/video-store-offline-/sw.js
